...
const WORK_SUB_CATEGORIES = [
    { suffix: 'characters', name: tt('人物设定', 'Characters', 'Персонажи'), category: 'character', type: 'folder', subFolders: [
        { name: tt('主要角色', 'Main Characters', 'Главные персонажи'), icon: 'Star' },
        { name: tt('次要角色', 'Supporting Characters', 'Второстепенные персонажи'), icon: 'User' },
        { name: tt('阵营/势力', 'Factions', 'Фракции'), icon: 'Shield' },
    ]},
    { suffix: 'locations', name: tt('空间/地点', 'Places', 'Места'), category: 'location', type: 'folder', subFolders: [
        { name: tt('主要场景', 'Main Locations', 'Основные места'), icon: 'Building' },
        { name: tt('自然环境', 'Natural Environment', 'Природная среда'), icon: 'Mountain' },
    ]},
    { suffix: 'world', name: tt('世界观/设定', 'Worldbuilding', 'Мир'), category: 'world', type: 'folder', subFolders: [
        { name: tt('历史/纪元', 'History / Eras', 'История / Эпохи'), icon: 'BookOpen' },
        { name: tt('社会/政治', 'Society / Politics', 'Общество / Политика'), icon: 'Crown' },
        { name: tt('文化/习俗', 'Culture / Customs', 'Культура / Обычаи'), icon: 'Compass' },
        { name: tt('力量体系', 'Power System', 'Система силы'), icon: 'Zap' },
    ]},
    { suffix: 'objects', name: tt('物品/道具', 'Items', 'Предметы'), category: 'object', type: 'folder', subFolders: [
        { name: tt('武器/装备', 'Weapons / Equipment', 'Оружие / Снаряжение'), icon: 'Sword' },
        { name: tt('特殊道具', 'Special Items', 'Особые предметы'), icon: 'Gem' },
    ]},
    { suffix: 'plot', name: tt('大纲', 'Outline', 'План'), category: 'plot', type: 'folder', subFolders: [
        { name: tt('主线', 'Main Plot', 'Основной сюжет'), icon: 'Flame' },
        { name: tt('支线', 'Subplots', 'Побочные линии'), icon: 'Feather' },
        { name: tt('伏笔', 'Foreshadowing', 'Предвестия'), icon: 'Lightbulb' },
    ]},
    { suffix: 'rules', name: tt('写作规则', 'Writing Rules', 'Правила'), category: 'rules', type: 'folder', subFolders: [
        { name: tt('文风规范', 'Style Guide', 'Стилистика'), icon: 'Palette' },
        { name: tt('禁忌/注意', 'Taboos / Notes', 'Табу / Примечания'), icon: 'Flag' },
    ]},
    { suffix: 'custom', name: tt('自定义设定', 'Custom Settings', 'Пользовательские настройки'), category: 'custom', type: 'folder', subFolders: [] },
];

// ── 内置默认名的显示层 i18n（2-way lookup）──
const FOLDER_LABEL_GROUPS = [
    ['人物设定', 'Characters', 'Персонажи'],
    ['空间/地点', 'Places', 'Места'],
    ['世界观/设定', 'Worldbuilding', 'Мир'],
    ['物品/道具', 'Items', 'Предметы'],
    ['大纲', 'Outline', 'План'],
    ['写作规则', 'Writing Rules', 'Правила'],
    ['自定义设定', 'Custom Settings', 'Пользовательские настройки'],
...
const BUILTIN_FOLDER_LABELS = Object.fromEntries(
    FOLDER_LABEL_GROUPS.map(([zh, en, ru]) => [zh, [en, ru]])
);

const BUILTIN_WORK_NAME_LABELS = Object.fromEntries(
    WORK_NAME_GROUPS.map(([zh, en, ru]) => [zh, [en, ru]])
);

export function getBuiltInFolderLabel(name, text) {
    if (!name || typeof text !== 'function') return name;
    for (const [zh, en, ru] of FOLDER_LABEL_GROUPS) {
        if (name === zh || name === en || name === ru) return text(zh, en, ru);
    }
    return name;
}
...
export function getBuiltInWorkName(name, text) {
    if (!name || typeof text !== 'function') return name;
    for (const [zh, en, ru] of WORK_NAME_GROUPS) {
        if (name === zh || name === en || name === ru) return text(zh, en, ru);
    }
    return name;
}
...
function getDefaultWorkNodes(workId) {
    ...
    nodes.push({
        id: `${wid}-bookInfo`, name: tt('作品信息', 'Book Info', 'Информация'), type: 'special', category: 'bookInfo',
...
async function migrateGlobalToPerWork() {
    ...
    if (!oldNodes || oldNodes.length === 0) {
        // 全新用户 → 创建默认作品
        const { workNode, subNodes } = createWorkNode(tt('默认作品', 'Default Work', 'Проект по умолчанию'), 'work-default');
    ...
    try {
        ...
        if (!biNode) {
            nodes.push({
                ...
                name: tt('作品信息', 'Book Info', 'Информация'), type: 'special', category: 'bookInfo',
...
async function migrateToWorkStructure(nodes) {
    ...
    const { workNode } = createWorkNode(tt('默认作品', 'Default Work', 'Проект по умолчанию'), 'work-default');
